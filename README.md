...
When the page loads with a `bookingUid` URL parameter, it sends a `vsl_opened` event to the n8n webhook at `https://n8n.adwaves.fr/webhook/vsl_clicked`. The request body is form-urlencoded and includes `event`, `firstname`, `datetime`, `bookingUid`, `openedAt`, `url`, `referrer`, and `userAgent`.
